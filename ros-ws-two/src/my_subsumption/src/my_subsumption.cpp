...
std_msgs::String Choosebehavior(){//choose next beahvior
	...
	if(go_near_beacon()){
		if(prev_behavior != "go_near_beacon"){
               std::cout <<prefix +  "ChoosenBehavior is go_near_red_beacon" <<std::endl;
		}			
	...
	if(order()){
		...
		std::cout <<prefix +  "ChoosenBehavior is order" <<std::endl;
		}		
		data.data = false;	
		scanEnable.publish(data);
		beaconEnable.publish(data);
	...
	if(deliver_order()){
		...
		std::cout << prefix + "ChoosenBehavior is deliver_order" <<std::endl;
		}			
	...
	if(pick_up_order()){
		...
		data.data = true;	
		beaconEnable.publish(data);
		std::cout << prefix +  "ChoosenBehavior is pick_up_order" <<std::endl;	
		}		
...
// read parameters
	...
	scanEnable = nh.advertise<std_msgs::Bool>(prefix + "sensors/hokuyoEnable", 1);
    	CurrentBehavior = nh.advertise<std_msgs::String>(prefix +  "curbehavior", 1);
	...
	ros::Subscriber poseSubs = nh.subscribe(prefix + "amcl_pose", 10, updatePose); //geometry_msgs/PoseWithCovarianceStamped
    //ros::Subscriber poseSubs = nh.subscribe("/amcl_pose", 10, updatePose); //geometry_msgs
    ros::Subscriber took_orderSubs =nh.subscribe(prefix + "took_order",1,took_func);
    ros::Subscriber RedcompleteSubs =nh.subscribe(prefix + "Red_gotoPoseCompleted",1,Red_gotoPoseCompleted);
    ros::Subscriber GreencompleteSubs =nh.subscribe(prefix + "Green_gotoPoseCompleted",1,Green_gotoPoseCompleted);
    ros::Subscriber PickedupSubs =nh.subscribe(prefix + "Pickedupobject",1,pickedupobject_func);
