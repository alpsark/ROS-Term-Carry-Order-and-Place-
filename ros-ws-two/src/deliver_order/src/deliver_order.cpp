...
void pass_func(const std_msgs::Bool::ConstPtr& BoolPointer ){
    if(BoolPointer->data ){
        pass = true;
	std::cout << prefix + "pass received0" << std::endl; 
    }else{
        pass = false;
    }
}
...
// read parameters
    ...
    completedPub = nh.advertise<std_msgs::Bool>( prefix + "Green_gotoPoseCompleted", 1);
    ros::Subscriber passSubs = nh.subscribe( "/Deliverpass", 5,pass_func);
    ros::Publisher passPub = nh.advertise<std_msgs::Bool>( "/Beaconpass", 1);
    float targetx;
    float targety ;
    float targetyaw;
    bool go_back = true; 
				 ...
				 if(go_back){
					...
					if (poses.size() == 2){
					   ...
					   if(poses.size() == 0){
						...
						//std::cout << prefix + "sending pass" << std::endl; 
