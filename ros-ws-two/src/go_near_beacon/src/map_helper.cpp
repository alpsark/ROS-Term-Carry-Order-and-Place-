...
bool MapHelper::isOccupied(int rowIndex, int colIndex)
{
	...
	for (int i = -7; i < 7; i++) {
		for (int j = -7; j < 7; j++) {
        		linearIndex = (rowIndex+i) * width + (colIndex+j);
...
bool MapHelper::isReallyOccupied(double pointx, double pointy)
{		
	//std::cout << "point: "  << pointx << std::endl ; 
	pointx = pointx - originx;
	pointy = pointy - originy;
	int rowIndex = floor(pointy/resolution);
	int colIndex = floor(pointx/resolution);
