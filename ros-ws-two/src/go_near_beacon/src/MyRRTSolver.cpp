...
#include <sstream>
#include <vector>
MyRRTSolver::MyRRTSolver(std::string px) {
	initialx = 0;
	initialy = 0;
	initialVertex = NULL;
...
// NOTE: you are supposed to add vertices and edges to graph data structure
  ...
  MyVertex* newValidVertex = new MyVertex(targetx, targety);
	...
	Nonvalid = (*mapHelper).isOccupied(newValidVertex->x , newValidVertex->y);
	 if(Nonvalid){	std::cout <<prefix<<  "Target Non valid " << std::endl; return ;}else {std::cout << prefix<<  "Target valid " << std::endl;}
  	//check validity of the inital
  newValidVertex = new MyVertex(initialx, initialy);
	Nonvalid = (*mapHelper).isOccupied(newValidVertex->x , newValidVertex->y);
	 if(Nonvalid){	std::cout << prefix<<"Initial Non valid " << std::endl; return ;}else {std::cout <<prefix<< "Initial valid " << std::endl;}
   int i = 0 ;
