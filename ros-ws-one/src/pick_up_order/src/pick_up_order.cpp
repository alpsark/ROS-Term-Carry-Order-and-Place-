  ...
  void pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg) {
	...
	if(!isCupPositionsAvailable){
		...
		for (int i = 0; i < pointCloud->points.size (); i++) {
			...
			else if( pointCloud->points[i].x-1.1 < -0.7  and pointCloud->points[i].x-1.1 >-1.12){
		...
		float measured_z = 0;
		for (int i = 0; i < pointCloud->points.size (); i++) {
		if(  pointCloud->points[i].y-0.949998 < -1.09 and pointCloud->points[i].y-0.949998 > -1.2 and pointCloud->points[i].x-1.1 <-1.11){continue;}
			else if( pointCloud->points[i].x-1.1 < -0.7  and pointCloud->points[i].x-1.1 >-1.12){
				if( pointCloud->points[i].y-0.949998 < -0.91 and pointCloud->points[i].y-0.949998 > -1.3){
		...
		if(cloudsize > 0  ){//greenbeacon Position :[ -1.1 , -0.949998 , 0.25 ]yaw 0
			...
			if(objectx > 0.38){//cylynder
		    	 	std::cout << "cyl " << std::endl ;
					shelfz = +8.5064e-01;
			}else if(objecty < -0.34){
				    std::cout << "cyl " << std::endl ;
					shelfz = +8.5064e-01;	
			}else{//cube
			...
			calculatedPosition.position.z = objectz - 0.0446423 + 0.2;
		}
	}
	}
 };
...
int main(int argc,char **argv)
{
  ...
  ros::Subscriber endEffectorPositionSubscriber = nh.subscribe<geometry_msgs::PoseStamped>("/UR10/positions/endEffectorPosition",1000,endEffectorPositionCallback);
   ...
   // ros::Subscriber cupPositionsSubscriber = nh.subscribe<geometry_msgs::PoseArray>("/task/positions/cups",1000,cupPositionaCallback);
    ...
    ros::Publisher PickedupPub =nh.advertise<std_msgs::Bool>("/Pickedupobject",1);
	//ros::Rate loop(10);
