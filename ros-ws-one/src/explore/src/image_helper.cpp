...
//std::cout << "r:" <<  (double) myImage(i,j)[2] << "g:" << (double) myImage(i,j)[1] << "b:" <<(double) myImage(i,j)[0] << " " ;
			...
			if ((double) myImage(i,j)[0] < 70 && (double) myImage(i,j)[1] > 180 && (double) myImage(i,j)[2] < 70) {
				greencentery += i;
				greencenterx += j;
				greenCount++;
			}
    ...
    //std::cout << "red " <<redCount << " green " <<greenCount <<std::endl;
	if (redCount > 10) {
		isredfound = true;
		redx = redcenterx / redCount;
		redx = redx - (cv_ptr->image.cols)/2 ;
		redx = redx / (cv_ptr->image.cols) ;
		//std::cout << "ball.x=" << centerx << " ball.y=" << centery << " redCount=" << redCount << std::endl;
		//std::cout << "redcenterx " << redx <<  std::endl;
	}

	if (greenCount > 10) {
		isgreenfound = true;
		greenx = greencenterx / greenCount;
		greenx = greenx - (cv_ptr->image.cols/2) ;
		greenx =  greenx / (cv_ptr->image.cols) ;
		//std::cout << "greencenterx " << greenx <<std::endl;
		//std::cout << "ball.x=" << centerx << " ball.y=" << centery << " redCount=" << redCount << std::endl;
	}
